  ...
  return (
    ...
    >
      <div className={'flex gap-3 items-center text-3xl justify-center row-span-2'}>
        ...
        <div>
          <div
            className={`rounded-full w-5 h-5 ${item.active ? (item.chargerStatus === ChargerStatus.CHARGING ? 'bg-green-500' : 'bg-red-500') : 'bg-black'} `}
          />
        </div>
      </div>
      <div
        className={`row-span-1 text-xl font-bold ${item.active ? (item.chargerStatus === ChargerStatus.CHARGING ? 'text-green-500' : 'text-red-500') : 'text-black'}`}
      >
